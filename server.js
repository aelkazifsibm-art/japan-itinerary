...
app.post('/api/nearby-pepites', async (req, res) => {
    try {
        ...
        for (const p of LIEUX_DB) {
            ...
            if (d <= radius_km) {
                results.push({
                    lat: p[0], lon: p[1], nom_fr: p[2], nom_jp: p[3],
                    ville: p[4], place_id: p[5]||null,
                    emoji: p[6], fatigue: p[7], type_fr: p[8]||'lieu secret',
...
app.post("/api/generate-program", async (req, res) => {
    try {
        ...
        const allDays = new Array(nb_days);
        let globalSummary = '';
...
JSON BRUT UNIQUEMENT (pas de markdown):
{
  ...
  "blocks": [
    ...
    {"type":"hotel_end","time":"20:00","title":"Retour hotel","duration_minutes":0}
            ...
            // CRITIQUE: forcer day_index correct (l'IA retourne souvent 0)
...
app.post("/api/resolve-place", async (req, res) => {
    try {
        ...
        // Cache RAM — évite de refaire le même appel Google Places
        const _ck = (query||'').toLowerCase().trim().slice(0, 80);
        if (!global._resolvePlaceCache) global._resolvePlaceCache = new Map();
        if (global._resolvePlaceCache.has(_ck)) {
            return res.json(global._resolvePlaceCache.get(_ck));
        }
        ...
        if (global._resolvePlaceCache && global._resolvePlaceCache.size < 500) {
            global._resolvePlaceCache.set(_ck, _result);
        }
        res.json(_result);
    } catch(e) {
